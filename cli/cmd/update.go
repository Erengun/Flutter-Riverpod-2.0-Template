...
func updateAppName() {
	...
	validateName := promptGetSelect(promptContent{
		label:   "Are you sure you want to update the app name to " + appName + "? (yes/no): ",
	...
	if appName != "" {
		...
		LoadingSpinner(stopChan, "Updating app name...")
...
func updatePackageName() {
	...
	validateName := promptGetSelect(promptContent{
		label:   "Are you sure you want to update the package name to " + packageName + "? (yes/no): ",
		choices: []string{"yes", "no"},
	})
	if validateName == "no" {
